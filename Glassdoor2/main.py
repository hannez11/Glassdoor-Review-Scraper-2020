...
parser.add_argument('-l', '--limit', default=100000,
...
formatter = logging.Formatter(
    ...
    'PROCESS2 %(asctime)s - %(message)s', "%H:%M:%S")
...
def get_browser():
    ...
    if args.headless: #dont set user-data-dir, otherwise chromium wont load for whatever reason. headless still uses cookies though
        chrome_options.add_argument('--remote-debugging-port=45448') #use different port for different processes
...
def main():
    ...
    if not args.start_from_url:
        ...
        browser.get(args.url)
        try:
            total_english_reviews = browser.find_element_by_css_selector('.col-6.my-0').text.split()[0]
        except:
